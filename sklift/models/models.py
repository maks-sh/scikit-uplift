...
class SDR(BaseEstimator):
    """
    Learn a classifier on new modified df:
    Feature matrix (X) =
    [ [T ,T, 0],
      [C, 0, C]]
    where:
        T = treatment group of train df,
        C = control group of train df,
        0 = matrix of '0' with appropriate shape

    Fit binary classifier with new feature matrix
    For uplift predictions use matrix:
    Feature matrix test treatment = [X, X, 0], (treatment predictions)
    Feature matrix test control   = [X, 0, X], (control predictions)
    where X = feature matrix on test df.

    Uplift is calculated as difference between treatment and control predictions
    returns uplift
    ...
    # ToDo: make Liner Model

    def __init__(self, estimator):
        self.estimator = estimator
        # check_estimator(estimator)

    def fit(self, X, y, treatment, estimator_fit_params=None, verbose=False):
        ...
        X_SDR = np.vstack((
            np.hstack((
                X_trmnt, X_trmnt, np.zeros(X_trmnt.shape)
            )),
            np.hstack((
                X_ctrl, np.zeros(X_ctrl.shape), X_ctrl
            ))
        ))

        y_sdr = np.hstack((y_trmnt, y_ctrl))

        if verbose:
            print('Train initial shape: ', X.shape)
            print('Train SDR matrix shape: ', X_SDR.shape)

        if estimator_fit_params is None: estimator_fit_params = {}
        self.estimator.fit(X_SDR, y_sdr, **estimator_fit_params)
        return self
    ...
    def predict(self, X):
        ...
        X_trmnt = np.hstack((X, X, np.zeros(X.shape)))
        X_ctrl = np.hstack((X, np.zeros(X.shape), X))

        self.trmnt_proba_ = self.estimator.predict_proba(X_trmnt)[:, 1]
        self.ctrl_proba_ = self.estimator.predict_proba(X_ctrl)[:, 1]
